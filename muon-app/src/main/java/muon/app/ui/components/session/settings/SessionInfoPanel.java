...
public class SessionInfoPanel extends JPanel {
    ...
    public void setSessionInfo(SessionInfo info) {
        this.info = info;
        inpHostName.setText(info.getHost());
        portModel.setValue(info.getPort());
        inpLocalFolder.setText(info.getLocalFolder());
        inpRemoteFolder.setText(info.getRemoteFolder());
        inpUserName.setText(info.getUser());
        inpPassword.setText(new String(
                info.getPassword() == null ? new char[0] : info.getPassword().toCharArray()
        ));
        inpKeyFile.setText(info.getPrivateKeyFile());

        cmbProxy.setSelectedIndex(info.getProxyType());
        inpProxyHostName.setText(info.getProxyHost());
        proxyPortModel.setValue(info.getProxyPort());
        inpProxyUserName.setText(info.getProxyUser());
        inpProxyPassword.setText(new String(
                info.getProxyPassword() == null ? new char[0] : info.getProxyPassword().toCharArray()
        ));

        setJumpHostDetails(info.getJumpType(), info.getJumpId());
        panPF.setInfo(info);
    }
    ...
    private JPanel createJumpPanel() {
        ...
        radMultiHopTunnel = new JRadioButton("Use SSH tunnel");
        radMultiHopPortForwarding = new JRadioButton("Use port forwarding");
        ...
        jumpHostPanel = new JumpHostPanel();
